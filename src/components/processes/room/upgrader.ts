...
class UpgraderProcess extends Process {
    ...
    public static start(roomName: string, parentPID: number) {
        const p = new UpgraderProcess(0, parentPID);
        addProcess(p);
    ...
    public run(): number {
        ...
        const upgraderName = memory.name;
    ...
    public runCreep(creepName: string): number {
        ...
        if (creep.pos.inRangeTo(controller.pos, 3) &&
            ...
            const pos = _.filter(storage.pos.adjacentPositions(), (pos) => pos.inRangeTo(controller.pos, 3));
            if (pos.length > 0)
                creep.moveTo(pos[0]);
            else {
    ...
    private runCourier(creepName: string): number {
        const creep = Game.creeps[creepName];
        const storage: StructureStorage | StructureContainer | null = Game.getObjectById(this.memory.containerID!) as StructureStorage | StructureContainer;
        if (!storage) {
            return 0;
        }
    ...
    private spawnCreep() {
        ...
        energy = energy - 50;
        const multiplier = Math.floor(energy / 250);
        if (spawnProcess) {
    ...
    private spawnCourier() {
        const roomName = this.memory.roomName;
        const spawnProcess = getSpawnProcess(this.memory.roomName);
        const energy = Game.rooms[roomName].energyCapacityAvailable;
        const multiplier = Math.floor(energy / 100);
        if (spawnProcess) {
